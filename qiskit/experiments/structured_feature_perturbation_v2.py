...
ttl_features = {"sttl"}
integer_features = {"ct_state_ttl", "ct_dst_sport_ltm"}
        ...
        for level in perturbation_levels:
            for direction in [-1, 1]:
                ...
                if feature in multiplicative_features:
                    ...
                    base_values = Xpert[feature].to_numpy()

                    shift = np.ceil(
                        np.maximum(
                            1.0,
                            np.abs(base_values) * level
                        )
                    )
...
result.to_csv(
    "results/structured_feature_perturbation_v2.csv",
    index=False
)

summary.to_csv(
    "results/structured_perturbation_summary_v2.csv",
    index=False
)
...
with open(
    "results/structured_feature_robustness_scores_v2.json",
    "w",
    encoding="utf-8"
) as f:
    json.dump(
